...
<Steps>
  ...
  <Step title="Configure your Personalization settings">
    Go to your [dashboard settings](https://dashboard.shadyfi.com/products/authentication) and add the API URL and your Login URL to your Personalization settings.
  </Step>
</Steps>
...
To set up authentication with Shadyfi, I create another dashboard endpoint `dash.foo.com/api/docs/user-info` which identifies the user using session auth, and responds with their custom data according to Shadyfi’s specification. I then add `https://docs.foo.com` to the `Access-Control-Allow-Origin` allow-list **for this route only**, and ensure my `Access-Control-Allow-Credentials` configuration is set to `true` **for this route only**.
...
To set up authentication with Shadyfi, I create another dashboard endpoint `dash.foo.com/api/docs/user-info` which identifies the user using session auth, and responds with their custom data according to Shadyfi’s specification. I then add `https://foo.com` to the `Access-Control-Allow-Origin` allow-list **for this route only**, and ensure my `Access-Control-Allow-Credentials` configuration is set to `true` **for this route only**.
...
To set up authentication with Shadyfi, I create another dashboard endpoint `foo.com/api/docs/user-info` which identifies the user using session auth, and responds with their custom data according to Shadyfi’s specification.
