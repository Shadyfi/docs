...
<Steps>
  <Step title="Generate a private key">
    Go to your [dashboard settings](https://dashboard.shadyfi.com/products/authentication) and generate a private key. Store this key somewhere secure where it can be accessed by your backend.
  </Step>
  ...
  <Step title="Configure your Authentication settings">
    Return to your [dashboard settings](https://dashboard.shadyfi.com/products/authentication) and add the login URL to your Authentication settings.
  </Step>
</Steps>
...
To set up authentication with Shadyfi, I go to my Shadyfi dashboard and generate a
...
I create a JWT containing the user’s custom data according to Shadyfi’s specification.
I use a JWT library to sign this JWT with my Shadyfi secret, create a redirect URL of the
...
const signingKey = await jose.importPKCS8(process.env.SHADYFI_PRIVATE_KEY, 'EdDSA');
...
private_key = os.getenv(SHADYFI_JWT_PEM_SECRET_NAME, '')

@router.get('/auth')
async def return_shadyfi_auth_status(current_user):
...
When an unauthenticated user tries to access a specific page, Shadyfi preserves their intended destination through a redirect flow:

1. The user attempts to visit a certain page (e.g., `/quickstart`)

2. Shadyfi redirects them to your login URL and adds the (relative) original destination as a `redirect` query parameter
