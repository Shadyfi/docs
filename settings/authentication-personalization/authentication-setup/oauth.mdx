...
If you have an existing OAuth server, you can integrate with Shadyfi for a seamless login experience.

## Implementation

<Steps>
  <Step title="Configure your Authentication settings">
    Go to your [Shadyfi authentication settings](https://dashboard.shadyfi.com/products/authentication), select the OAuth option, and fill out the required fields:
  ...
  <Step title="Configure your OAuth client">
    Copy the Redirect URL listed in the [Shadyfi authentication settings](https://dashboard.shadyfi.com/products/authentication) and add it as an authorized redirect URL for your OAuth server.
  </Step>
  <Step title="Create your Info API (Optional)">
    ...
    Return to your [Shadyfi authentication settings](https://dashboard.shadyfi.com/products/authentication) and add the Info API URL
...
To set up authentication with Shadyfi, I create an endpoint `api.foo.com/docs/user-info` which requires an OAuth access token with the `docs-user-info` scope, and responds with the user's custom data according to Shadyfi’s specification.
