...
You can configure the CI checks enabled for a deployment on the Shadyfi dashboard by navigating to the 'Add-Ons' tab. There you can enable or disable the checks you'd like to run.
...
Shadyfi supports automatically running Vale in a CI check, and displaying the results as a check status.
...
<Tip>Don't have a Vale config or not sure where to get started? Don't worry, Shadyfi has a default configuration that will automatically be used if one is not provided.</Tip>
...
{/* <!-- vale on --> */}
```
Please note that these commment tags are not supported within Shadyfi components, but can be used anywhere at the base level of a document.