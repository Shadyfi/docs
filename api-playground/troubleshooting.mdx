...
<AccordionGroup>
  <Accordion title="All of my OpenAPI pages are completely blank">
    In this scenario, it's likely that either Shadyfi cannot find your OpenAPI document,
    or your OpenAPI document is invalid.

    Running `Shadyfi dev` locally should reveal some of these issues.
    ...
    This is the exact validation package Shadyfi uses to validate OpenAPI documents, so if your document
    passes validation here, there's a great chance the problem is elsewhere.

    Additionally, Shadyfi does not support OpenAPI 2.0. If your document uses this version of the specification,
...
```
  ...
  <Accordion title="Requests from the API Playground don't work">
    ...
    Shadyfi to send requests directly to your backend with the `api.playground.disableProxy`
